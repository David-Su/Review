...
public class ThreadPoolExecutor extends AbstractExecutorService {
     ...
     */
    private boolean addWorker(Runnable firstTask, boolean core) { //核心work有firstTask，非核心没有
        retry:
        for (;;) {
            int c = ctl.get();
            int rs = runStateOf(c);
            ...
            if (rs >= SHUTDOWN &&
                ! (rs == SHUTDOWN &&
                   firstTask == null &&             //                  |-----状态大于SHUTDOWN
                                                    //                  |
                   ! workQueue.isEmpty()))         //状态大于等于SHUTDOWN |-----firstTask!=null      //满足的话返回失败
                return false;                      //                   |
                                                   //                   |
                                                   //                   |------workQueue是空的
        ...
        try {
            ...
            if (t != null) {
                ...
                try {
                    ...
                    if (rs < SHUTDOWN ||
                        (rs == SHUTDOWN && firstTask == null)) { //现在是运行状态或者是SHUTDOWN状态而且firstTask是空
    ...
    public void execute(Runnable command) {
         ...
         */
        int c = ctl.get();
        if (workerCountOf(c) < corePoolSize) { //work的数量小于核心线程
            if (addWorker(command, true))
                return;
            c = ctl.get();
        }
        //work的数量大于等于核心线程，或者addWorker失败
        if (isRunning(c) && workQueue.offer(command)) { //如果正在运行 && 从workQueue添加任务成功
            int recheck = ctl.get();
            if (!isRunning(recheck) && remove(command))  //如果不在运行，从workQueue移除任务
                reject(command);
            else if (workerCountOf(recheck) == 0) //如果正在运行或者workQueue移除任务失败，而且work数量为0
                addWorker(null, false); //加一个非核心work
        }
        else if (!addWorker(command, false)) //如果不在运行 || 从workQueue添加任务失败，//加一个非核心work
            reject(command);
    }
